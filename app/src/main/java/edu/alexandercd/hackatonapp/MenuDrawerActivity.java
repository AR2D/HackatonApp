...
public class MenuDrawerActivity extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.item_profeco) {
            fragment = new SearchFragment();
            fragmentSelected = true;
        } else if (id == R.id.item_lista) {
            fragment = new SearchFragment();
            fragmentSelected = true;
        } else if (id == R.id.item_nose) {
            fragment = new SearchFragment();
            fragmentSelected = true;
        } else if (id == R.id.item_nose2) {
        }
        if (fragmentSelected) {
            getSupportFragmentManager().beginTransaction().replace(R.id.content_menu_drawer, fragment).commit();
        }
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.drawer_layout);
        drawer.closeDrawer(GravityCompat.START);
        return true;
